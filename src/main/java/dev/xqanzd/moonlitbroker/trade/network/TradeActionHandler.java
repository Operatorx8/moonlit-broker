...
import dev.xqanzd.moonlitbroker.mixin.MerchantScreenHandlerAccessor;
import dev.xqanzd.moonlitbroker.registry.ModItems;
import dev.xqanzd.moonlitbroker.trade.SecretGateValidator;
...
public class TradeActionHandler {
    ...
    private static void handleRefresh(ServerPlayerEntity player, MysteriousMerchantEntity merchant, int requestedPageIndex) {
        ...
        if (haveBefore < REFRESH_SCROLL_COST) {
            player.sendMessage(
                    Text.translatable("message.xqanzd_moonlit_broker.trade.refresh.deny",
